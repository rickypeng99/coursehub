CREATE OR REPLACE VIEW `must` AS (SELECT Users_Skills.net_id
 ...
 CREATE OR REPLACE VIEW `all` AS (SELECT Users_Skills.net_id, ROUND((COUNT(*) / (SELECT COUNT(*) FROM Groups_Skills WHERE Groups_Skills.group_id = 3) * 100), 0) AS skill_ratio
